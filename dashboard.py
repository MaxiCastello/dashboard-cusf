...
try:
    ...
    if columnas_necesarias.issubset(set(penales.columns)):
        ...
        # Calcular el total por situación
        ...
        # Formatear etiquetas
        penales["label a favor"] = penales["a favor"].astype(str) + " (" + penales["% a favor"].astype(str) + "%)"
        penales["label en contra"] = penales["en contra"].astype(str) + " (" + penales["% en contra"].astype(str) + "%)"
        ...
        labels = penales[["situacion", "label a favor", "label en contra"]].copy()
        labels = labels.melt(id_vars="situacion", var_name="tipo", value_name="label")
        labels["tipo"] = labels["tipo"].replace({
            "label a favor": "a favor",
            "label en contra": "en contra"
            })
        ...
        fig_apiladas = px.bar(
            ...
            color_discrete_map={
            "a favor": "#28A745",   # Verde
            "en contra": "#C0392B"  # Rojo
            },
            height=600
            )
