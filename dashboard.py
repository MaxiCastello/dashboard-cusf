...
def generar_informe_pdf(
    ...
    pen_imgs = [fig_to_img(f, width_pt=290) for f in pen_imgs]
    if pen_imgs:
        story.append(Paragraph("Penales", H2))
        rows = [pen_imgs[i:i+2] for i in range(0, len(pen_imgs), 2)]
        for r in rows:
            story.append(Table([r], colWidths=[290]*len(r), hAlign="LEFT"))
            story.append(Spacer(1, 6))
    ...
    for titulo_secc, trio in [
        ...
        if trio:
            ...
            story.append(Table([trio], colWidths=[230]*len(trio), hAlign="CENTER"))
            story.append(Spacer(1, 6))
