...
DASHBOARD_CSS = """
<style>
/* Espaciado general para que no se corte el H1 */
.block-container { padding-top: 1.2rem; padding-bottom: .8rem; }
...
/* Títulos de tarjetas: mismos altos para alinear filas */
.card h4{
  margin:0 0 6px 0;font-weight:700;min-height:28px;
  display:flex;align-items:center
}

/* KPI: centrados y más “fuertes” */
.kpi{
  background:#202226;border:1px solid #2e3136;border-radius:14px;
...
/* Leyendas Plotly un poco más próximas */
.js-plotly-plot .legend{ margin-top:-4px }

/* Selectbox compacto y en línea dentro de las tarjetas switch */
.stSelectbox > div { display:inline-block; min-width:220px; }
.stSelectbox label { display:none; }  /* ocultar label vacío */
</style>
"""
...
def switch_card(titulo, opciones: dict, default_key=None, height=260, compact=True):
    if not opciones:
        st.info("Sin datos.")
        return
    ...
    col_tit, col_sel = st.columns([0.62, 0.38])
    with col_tit:
