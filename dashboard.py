...
if archivos:
    ...
    for archivo in archivos:
        ...
        with st.expander(f"📁 Datos del archivo: {os.path.basename(archivo)}", expanded=expandir_todo):
            jugadores_completos = pd.DataFrame({"nombre completo": resumen["nombre completo"].unique()})
            resumen = jugadores_completos.merge(resumen, on="nombre completo", how="left").fillna(0)
