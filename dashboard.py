...
h1, .st-emotion-cache-10trblm { margin-top: 0 !important; }
...
.kpi{background:#202226;border:1px solid #2e3136;border-radius:14px;padding:14px 16px;text-align:center}
.kpi .val{font-size:28px;font-weight:700;line-height:1}
.kpi .lbl{opacity:.85;font-size:13px;margin-top:6px}

/* leyendas plotly un poco más próximas al gráfico */
.js-plotly-plot .legend{margin-top:-4px}

/* selectboxes compactos y en línea (evita ese "bloque" largo) */
    ...
    if 'fig_total' in locals() and fig_total is not None:
       fig_total.update_layout(height=(h_small + 120),
                               margin=dict(l=140 if not modo_celular else 90,
                                        r=40, t=40, b=10))
       left, mid, right = st.columns([0.10, 0.80, 0.10])   # centrado
       with mid:
