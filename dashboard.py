...
try:
    ...
    if {"situacion", "propios", "rival", "motivo"}.issubset(set(penales.columns)):
        ...
        st.plotly_chart(fig1, use_container_width=True, key="grafico_resumen")
        
        total_penales = {'Tipo': ['rival', 'propios'], 'Cantidad': [150, 206]}
        df_total = pd.DataFrame(total_penales)

        fig_donut = px.pie(df_total, values='Cantidad', names='Tipo',
                   title='Proporción de Penales A Favor vs En Contra',
                   hole=0.5, color_discrete_sequence=['#D13248', '#3733C7'])

        st.plotly_chart(fig_donut, use_container_width=True, key="grafico_dona")
        

        st.markdown("""
                **📝 Análisis del gráfico 1:**

        De este gráfico podemos observar que se cobraron un total de **356 penales** a lo largo de **15 partidos**.  
        De esos, **206 fueron en contra nuestra** y **150 a favor**, lo que da un promedio de:

            - **14 penales cometidos por partido**
            - **10 penales a favor por partido**

        Esto marca una diferencia de 4 penales más en contra por encuentro.
        """)
        ...
        st.plotly_chart(fig2, use_container_width=True, key="grafico_ruck")
        ...
        st.plotly_chart(fig3, use_container_width=True,  key="grafico_juego")
