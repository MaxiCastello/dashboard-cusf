...
archivos = [
    ...
    if archivo.endswith(".xlsx") and not archivo.lower().startswith("Estadistica")
    ]
...
try:
    ...
    penales = pd.read_excel("Estadistica.xlsx", sheet_name="Penales")
    
    # Asegurar que las columnas necesarias estén presentes
    columnas_necesarias = {"Situación", "A favor", "En contra"}
    if columnas_necesarias.issubset(set(penales.columns)):

        # Calcular total por situación (para ordenar)
        penales["Total"] = penales["A favor"] + penales["En contra"]
        penales = penales.sort_values("Total", ascending=True)

        ## 🔹 Gráfico 1: Barras horizontales por situación
        st.subheader("📊 Total de Penales por Situación")
        fig1 = px.bar(
            penales,
            y="Situación",
            x="Total",
            orientation="h",
            title="Penales Totales por Situación (A favor + En contra)",
            labels={"Total": "Cantidad de Penales"},
            height=600,
            text="Total"
        )
        fig1.update_layout(margin=dict(l=120, r=40, t=50, b=50))
