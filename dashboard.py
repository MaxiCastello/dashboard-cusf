...
try:
    ...
    if {"situacion", "a favor", "en contra"}.issubset(set(penales.columns)):
        ...
        # Agrupación manual por categoría principal
        condiciones = {
            "Ruck": ["retener pelota", "pesca indevida", "estorbar el juego", "entrar por el costado", "tirarse de cabeza", "Offside","entrada imprudente","otro"],
            "Juego": ["defensa en offside", "pantalla", "tackle alto", "tackle peligroso", "offside en kick", "knock-on intencional","inconducta","otro"],
            "Fijos": ["scrum", "line"],
            "Kick": ["salidas", "salidas de 22"],
        }

        def categorizar(situacion):
            s = situacion.lower()
            for categoria, palabras in condiciones.items():
                if any(p in s for p in palabras):
                    return categoria
            return "Otros"

        penales["categoria"] = penales["situacion"].apply(categorizar)

        # Gráfico 1
        resumen = penales.groupby("categoria")[["a favor", "en contra"]].sum().reset_index()
        st.subheader("📊 Penales Totales por Categoría Principal")
        ...
        fig1 = px.bar(
            ...
            title="Penales a Favor y En Contra por Categoría",
            height=500
        )
        st.plotly_chart(fig1, use_container_width=True)

        # Gráfico 2: Ruck
        st.subheader("Detalle de Penales en Ruck")
        penales_ruck = penales[penales["categoria"] == "Ruck"]
        ...
        # Gráfico 3: Juego
        st.subheader("Detalle de Penales en Juego")
        penales_juego = penales[penales["categoria"] == "Juego"]
        ...
        fig3 = px.bar(
            ...
            barmode="group",
            labels={"value": "Cantidad", "variable": "Tipo"},
            title="Detalle de Penales en Juego",
            color_discrete_map={"a favor": "#28A745", "en contra": "#C0392B"},
            height=500
        )
        ...
        st.warning("❗ Faltan columnas esperadas: 'Situación', 'A favor', 'En contra'")

except Exception as e:
    st.error(f"Error al procesar los datos de la hoja 'Penales': {e}")