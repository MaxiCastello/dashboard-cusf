...
def generar_informe_pdf(
    ...
    pen_imgs = [f for f in [figs.get("pen_situaciones"), figs.get("pen_ruck"),
                        figs.get("pen_juego"), figs.get("pen_scrum")] if f]
    pen_imgs = [fig_to_img(f, width_pt=270) for f in pen_imgs]
    if pen_imgs:
        story.append(Paragraph("Penales", H2))
        rows = [pen_imgs[i:i+2] for i in range(0, len(pen_imgs), 2)]
        for r in rows:
            story.append(Table([r], colWidths=[270]*len(r), hAlign="LEFT"))
            story.append(Spacer(1, 6))
    ...
    for titulo_secc, trio in [
        ...
        trio = [fig_to_img(f, width_pt=195) for f in trio if f]
        if trio:
            story.append(Paragraph(titulo_secc, H2))
            story.append(Table([trio], colWidths=[195]*len(trio), hAlign="LEFT"))
            story.append(Spacer(1, 6))
    ...
    if figs.get("efectividad22"):
        ...
        story.append(fig_to_img(figs["efectividad22"], width_pt=500))
        story.append(Spacer(1, 6))
        if conclusion_22:
...
try:
    ...
    if fila_tot.any():
        ...
        total_pen_propios = int(penales["propios"].fillna(0).sum())  # fallback

        # cantidad de partidos hoja info
        info_df = pd.read_excel(archivo_estadistica, sheet_name="Info")
        partidos_pen = int(info_df.loc[info_df["Variable"].str.lower() == "cantidad_partidos", "Valor"].iloc[0])
        ...
        texto_conclusion_penales = (
            ...
            f"<b>{partidos_pen}</b> partidos, lo que da un promedio de "
            f"<b>{prom_pen:.1f}</b> por partido."
        )
