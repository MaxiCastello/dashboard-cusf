...
def generar_informe_pdf(
    ...
    PAGE_W, PAGE_H = A4
    FRAME_W = PAGE_W - doc.leftMargin - doc.rightMargin   # 539 pt
    COL_GAP  = 8                                          # separación entre gráficos

    W_FULL  = FRAME_W                                     # una sola imagen
    W_HALF  = (FRAME_W - COL_GAP) / 2                     # dos por fila
    W_THIRD = (FRAME_W - 2*COL_GAP) / 3                   # tres por fila
    ...
    if figs.get("puntos_acc"):
        story.append(fig_to_img(_for_pdf(figs["puntos_acc"]), width_pt=W_FULL))
        story.append(Spacer(1, 8))
