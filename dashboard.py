...
.kpi .val{font-size:34px;font-weight:800;line-height:1;letter-spacing:.2px}
.kpi .lbl{opacity:.9;font-size:13px;margin-top:6px}

.card{background:#202226;border:1px solid #2e3136;border-radius:14px;
      padding:12px 14px;box-shadow:0 2px 10px rgba(0,0,0,.25)}
.header-row{display:flex;gap:12px;align-items:center;justify-content:space-between;margin:2px 2px 8px 2px}
.header-row h3,.header-row h4,.header-row h2{margin:0;font-weight:700}
...
def header_with_select(title:str, options:list, key:str, default=None):
    ...
    with col_t: st.markdown(f"### {title}")
    with col_s: st.selectbox(" ", options, index=(options.index(default) if default in options else 0),
...
try:
    ...
    if 'fig_total' in locals() and fig_total is not None:
        fig_total.update_layout(height=(h_small + 120),
