    ...
    for archivo in archivos:
        ...
        with st.expander(f"📁 Datos del archivo: {os.path.basename(archivo)}", expanded=expandir_todo):
            ...
            df_plot["texto"] = df_plot.apply(
                lambda row: row["etiqueta"] if row["resultado"] == "errados" and row["cantidad"] > 0 else "", axis=1
                )
            df_plot["texto"] = df_plot.apply(
                lambda row: row["etiqueta"] if row["resultado"] == "tackles" and row["texto"] == "" and row["cantidad"] > 0                                         else row["texto"],
                axis=1
                )
