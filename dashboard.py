...
DASHBOARD_CSS = """
<style>
.block-container{padding-top:1.6rem;padding-bottom:.8rem}
h1 { margin-top: 0 !important; }

.kpi{
  background:#202226;border:1px solid #2e3136;border-radius:14px;
  padding:14px 16px;text-align:center;
  display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:84px;
}
...
.stSelectbox > div {display:inline-block; min-width: 220px;}
...
def switch_card(titulo, opciones: dict, default_key=None, height=260, key=None):
    if not opciones:
        st.info("Sin datos.")
        return
    ...
    # Encabezado compacto y alineado (evita "undefined")
    col = st.container()
    with col:
        st.markdown(f'<div class="switch-head"><h4>{titulo}</h4>', unsafe_allow_html=True)
        choice = st.selectbox("", keys, index=keys.index(default_key),
                              label_visibility="collapsed", key=f"{(key or titulo)}_sel")
        st.markdown('</div>', unsafe_allow_html=True)
...
def header_with_select(title:str, options:list, key:str, default=None):
    ...
    with col_t:
        st.markdown(f"### {title}")
    with col_s:
        _ = st.selectbox(" ", options, index=(options.index(default) if (default in options) else 0),
    ...
    if 'fig_total' in locals() and fig_total is not None:
       fig_total.update_layout(height=(h_small + 120),
                               margin=dict(l=140 if not modo_celular else 90,
                                        r=40, t=40, b=10))
       left, mid, right = st.columns([0.7, 0.75, 0.7])
       with mid:
