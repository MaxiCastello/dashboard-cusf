...
.header-row h3{margin:0;font-size:1.1rem;font-weight:700}
...
try:
    ...
    for f in (fig_f, fig_c):
        f.update_layout(title=dict(y=0.98, x=0.5))
        f.update_layout(
    ...
    # 3) Fila media: Line / Scrum / Penales (dejamos Efectividad 22 para la fila siguiente)
    c1, c2, c3 = grid(3)
    with c1:
        switch_card("Line",
    ...
    # 5) TACKLES centrado (fila sola, más ancho)
