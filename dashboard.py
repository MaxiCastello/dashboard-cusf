...
if archivos:
    ...
    for archivo in archivos:
        ...
        for col in resumen.columns:
            if "nombre" in col and "nombre del jugador" in col:
                resumen.rename(columns={col: "nombre del jugador"}, inplace=True)

        with st.expander(f"📁 Datos del archivo: {os.path.basename(archivo)}", expanded=expandir_todo):
            jugadores_completos = pd.DataFrame({"nombre del jugador": list(map(str, range(1, 26)))})
            resumen["nombre del jugador"] = resumen["nombre del jugador"].astype(str)
            resumen = jugadores_completos.merge(resumen, on="nombre del jugador", how="left").fillna(0)
            ...
            st.subheader("📈 Gráfico de Tackles por partido por nombre de jugador")
            ...
            fig = px.bar(
                ...
                category_orders={"nombre del jugador": list(map(str, range(1, 26)))},
                text="texto"
                )
