...
with st.sidebar:
    vista = st.radio(
        "Navegación",
        ["Tablero", "Tackles", "Penales", "Line", "Scrum", "Salidas", "Salidas 22", "Efectividad 22", "Puntos"],
        index=0,
    )
SHOW_SECCIONES = (vista != "Tablero")

# =========================
# HELPERS (texto, cards, grid)
# =========================
...
if archivos:
    ...
    # =========================
    # GLOBAL: TACKLES + RESTO DE SECCIONES (si corresponde)
    # =========================
    if "nombre del jugador" in resumen_total.columns:
        ...
        # Figura total (usada también en Tablero)
        ...
        # Donut por jugador (solo se muestra en sección Tackles, no en Tablero)
...
# (Se renderizan solo si la vista coincide o si se necesitan sus figuras)
# =========================
try:
    ...
    if {"situacion","propios","rival","motivo"}.issubset(set(penales.columns)):
        penales["situacion"] = penales["situacion"].astype(str).strip().str.lower()
        penales["motivo"] = penales["motivo"].astype(str).strip().str.lower()
        ...
        fig1 = px.bar(resumen, x="situacion", y=["propios","rival"], barmode="group",
                      ...
                      color_discrete_map={"propios":"%23%2328A745".replace("%23","#"), "rival":"#C0392B"},
    ...
    # =========================
    # TABLERO (la vista compacta tipo PowerBI)
    # =========================
    if vista == "Tablero":
        # Fila de KPIs
        c1,c2,c3,c4 = grid(4)
        ...
        c1,c2,c3 = grid(3)
        with c1:
            card("Total de puntos", lambda: st.plotly_chart(fig_bar, use_container_width=True, config={"displayModeBar": False}))
        with c2:
            card("Composición a favor", lambda: st.plotly_chart(fig_f, use_container_width=True, config={"displayModeBar": False}))
        with c3:
            card("Composición en contra", lambda: st.plotly_chart(fig_c, use_container_width=True, config={"displayModeBar": False}))

        # Fila Line / Scrum
        c1,c2,c3 = grid(3)
        if 'fig_overall' in locals():
            with c1: card("Line totales", lambda: st.plotly_chart(fig_overall, use_container_width=True, config={"displayModeBar": False}))
        if 'fig_overall_scrum' in locals():
            with c2: card("Scrum totales", lambda: st.plotly_chart(fig_overall_scrum, use_container_width=True, config={"displayModeBar": False}))
        with c3:
            card("Precisión (Conv/Pen)", lambda: st.plotly_chart(fig_acc, use_container_width=True, config={"displayModeBar": False}))

        # Fila Salidas
        if 'fig_salidas_total' in locals():
            c1,c2,c3 = grid(3)
            with c1: card("Salidas totales", lambda: st.plotly_chart(fig_salidas_total, use_container_width=True, config={"displayModeBar": False}))
            with c2: card("Salidas propias", lambda: st.plotly_chart(fig_salidas_propias, use_container_width=True, config={"displayModeBar": False}))
            with c3: card("Salidas rival",   lambda: st.plotly_chart(fig_salidas_rival, use_container_width=True, config={"displayModeBar": False}))

        # Fila Salidas 22
        if 'fig_total_22' in locals():
            c1,c2,c3 = grid(3)
            with c1: card("Salidas 22 totales", lambda: st.plotly_chart(fig_total_22, use_container_width=True, config={"displayModeBar": False}))
            with c2: card("Salidas 22 propias", lambda: st.plotly_chart(fig_propias_22, use_container_width=True, config={"displayModeBar": False}))
            with c3: card("Salidas 22 rival",   lambda: st.plotly_chart(fig_rival_22, use_container_width=True, config={"displayModeBar": False}))

        # Fila Efectividad 22 + Tackles Totales
        c1,c2 = grid(2)
        with c1: card("Efectividad en 22", lambda: st.plotly_chart(fig_eff, use_container_width=True, config={"displayModeBar": False}))
        if 'fig_total' in locals():
            with c2: card("Tackles Totales por jugador", lambda: st.plotly_chart(fig_total, use_container_width=True, config={"displayModeBar": False}))
    ...
    pdf_buffer = generar_informe_pdf(
        ...
        kpis=kpis, tabla_puntos=[
            ["Item","A favor","En contra"],
            ["Tries", int(rowp["tries_favor"]), int(rowp["tries_contra"])],
            ["Conversiones", f"{int(rowp['conv_favor_m'])}/{int(rowp['conv_favor_t'])}",
                              f"{int(rowp['conv_contra_m'])}/{int(rowp['conv_contra_t'])}"],
            ["Penales", f"{int(rowp['pen_favor_m'])}/{int(rowp['pen_favor_t'])}",
                        f"{int(rowp['pen_contra_m'])}/{int(rowp['pen_contra_t'])}"],
            ["Drops", int(rowp.get("drops_favor",0)), int(rowp.get("drops_contra",0))],
            ["Puntos", pf, pc],
        ],
        figs=figs,
        tackles_tabla='tabla_tackles' in locals() and tabla_tackles or None,
        conclusion_22=(
        ...
        conclusion_penales='texto_conclusion_penales' in locals() and texto_conclusion_penales or None,
    )
    st.download_button(
