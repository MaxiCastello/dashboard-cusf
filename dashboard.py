...
.kpi .val{font-size:38px;font-weight:900;line-height:1;letter-spacing:.2px}
.kpi .lbl{opacity:.95;font-size:14px;margin-top:6px}
...
def switch_card(opciones: dict, default_key=None, height=260, key="switch"):
    if not opciones:
        st.info("Sin datos.")
        return
    ...
    choice = st.selectbox(" ", keys, index=keys.index(default_key),
                          label_visibility="collapsed", key=f"{key}_sel")
...
try:
    ...
    with c3:
        ...
        switch_card(
            ...
            default_key="Totales", height=h_small, key="penales"
        )
        st.markdown("</div>", unsafe_allow_html=True)
