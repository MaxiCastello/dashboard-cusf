...
def tune_for_tv(fig, title_size=18, legend_y=-0.18, top=36, bottom=10):
    fig.update_layout(
        margin=dict(l=10, r=10, t=top, b=bottom),
        title=dict(font=dict(size=title_size)),
        legend=dict(orientation="h", x=0.5, xanchor="center", y=legend_y, yanchor="top"),
        paper_bgcolor="white", plot_bgcolor="white", template="plotly_white",
    )
    return fig

def render_grid(figs, cols=3, height=360):
    if not figs:
        return
    # asegurar altura en cada figura
    for f in figs:
...
modo_tv = st.toggle(
    ...
    key="modo_tv",
    value=False,
    help="Muestra muchos gráficos juntos en grilla para TV/proyector."
)
if modo_tv:
    st.markdown(CSS_TV, unsafe_allow_html=True)
...
def generar_informe_pdf(
    ...
    def colw(n, gap=8):  # ancho de cada columna para n gráficos por fila
        return (USABLE_W - gap*(n-1)) / n

    W_FULL  = USABLE_W            # una sola imagen
    W_HALF  = colw(2)             # dos por fila
    W_THIRD = colw(3) * 1.1       # tres por fila

    # Reduce márgenes internos y baja la leyenda
    ...
    if row_imgs:
        ...
        story.append(Table([row_imgs],
            colWidths=[W_THIRD]*len(row_imgs),
            hAlign="CENTER",
    ...
    if figs.get("puntos_acc"):
        story.append(Table(
            [[fig_to_img(_for_pdf(figs["puntos_acc"]), width_pt=W_FULL)]],
            colWidths=[W_FULL],
            hAlign="CENTER",
    ...
    # Penales (4 graficos - 2 por filas)
    ...
    if pen_imgs:
        ...
        for r in rows:
            story.append(Table([r],
                colWidths=[W_HALF]*len(r),
                hAlign="CENTER",
                style=[("LEFTPADDING",(0,0),(-1,-1),0), ("RIGHTPADDING",(0,0),(-1,-1),0)]
            ))
            story.append(Spacer(1, 6))
    ...
    for titulo_secc, trio in [
        ...
        if trio:
            ...
            story.append(Table([trio], colWidths=[W_THIRD]*len(trio), hAlign="CENTER",style=[("LEFTPADDING",(0,0),(-1,-1),0), ("RIGHTPADDING",(0,0),(-1,-1),0)]))
            story.append(Spacer(1, 6))

    # Salidas y Salidas 22 (de a 3)
    for titulo_secc, trio in [
...
try:
    ...
    # Modo TV / Proyector
    if modo_tv:
        # Tamaños de la visualizacion (subí H o bajá COLS para agrandar)
        H = 380     # altura de cada graf en la grilla
        COLS = 3    # columnas por fila (2 => más grandes)

        # Armamos listas por secciones solo con los figs que existan
        figs_puntos = [tune_for_tv(f, top=40) for f in [fig_bar, fig_f, fig_c] if f is not None]
        figs_line   = [tune_for_tv(f) for f in [fig_overall, fig_propios, fig_rival] if f is not None]
        figs_scrum  = [tune_for_tv(f) for f in [fig_overall_scrum, fig_propios_scrum, fig_rival_scrum] if f is not None]
        figs_sal    = [tune_for_tv(f) for f in [fig_salidas_total, fig_salidas_propias, fig_salidas_rival] if f is not None]
        figs_sal22  = [tune_for_tv(f) for f in [fig_total_22, fig_propias_22, fig_rival_22] if f is not None]
        figs_eff22  = [tune_for_tv(fig, title_size=18, top=50)]  # la línea de efectividad

        st.subheader("Puntos")
        render_grid(figs_puntos, cols=COLS, height=H)
    
        st.subheader("Line")
        render_grid(figs_line, cols=COLS, height=H)

        st.subheader("Scrum")
        render_grid(figs_scrum, cols=COLS, height=H)
        
        st.subheader("Salidas")
        render_grid(figs_sal, cols=COLS, height=H)

        st.subheader("Salidas 22")
        render_grid(figs_sal22, cols=COLS, height=H)

        st.subheader("Efectividad en 22")
        render_grid(figs_eff22, cols=COLS, height=H)
