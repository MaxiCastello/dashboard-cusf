...
try:
    ...
    columnas_necesarias = {"situacion", "a favor", "en contra"}
    if columnas_necesarias.issubset(set(penales.columns)):

        # Calcular el total por situación (sólo si no existe la columna)
        penales["total"] = penales["a_favor"] + penales["en_contra"]

        # Calcular los porcentajes (por situación)
        penales["% a favor"] = (penales["a_favor"] / penales["total"] * 100).round(1)
        penales["% en contra"] = (penales["en_contra"] / penales["total"] * 100).round(1)

        # Formatear etiquetas
        penales["label_a_favor"] = penales["a_favor"].astype(str) + " (" + penales["% a favor"].astype(str) + "%)"
        penales["label_en_contra"] = penales["en_contra"].astype(str) + " (" + penales["% en contra"].astype(str) + "%)"

        # Convertir a formato largo para usar con barras apiladas
        df_long = penales.melt(
            id_vars=["situacion"],
            value_vars=["a_favor", "en_contra"],
            var_name="tipo",
            value_name="cantidad"
            )

        # Etiquetas correspondientes
        labels = penales[["situacion", "label_a_favor", "label_en_contra"]].copy()
        labels = labels.melt(id_vars="situacion", var_name="tipo", value_name="label")
        labels["tipo"] = labels["tipo"].replace({
            "label_a_favor": "a_favor",
            "label_en_contra": "en_contra"
            })

        # Merge etiquetas con el dataframe largo
        df_long = df_long.merge(labels, on=["situacion", "tipo"], how="left")
        ...
        fig_apiladas = px.bar(
            ...
            text="label",
            orientation="h",
            title="Penales a Favor vs En Contra por Situación",
            color_discrete_map={
            "a_favor": "#28A745",   # Verde
            "en_contra": "#C0392B"  # Rojo
            },
            height=600
            )

        fig_apiladas.update_layout(
            xaxis_title="Cantidad de Penales",
            yaxis_title="Situación",
            legend_title="Tipo",
            margin=dict(l=120, r=40, t=60, b=50)
            )

        fig_apiladas.update_traces(textposition="inside", insidetextanchor="start")
        st.subheader("📊 Comparativa Penales a Favor vs En Contra")
        st.plotly_chart(fig_apiladas, use_container_width=True)

        ##  Gráfico 2: Barras apiladas A favor vs En contra
        st.subheader("📊 Penales a Favor vs En Contra")
        fig2 = px.bar(
            penales,
            x="situacion",
            y=["a favor", "en contra"],
        ...
        fig2.update_layout(xaxis_tickangle=-45, margin=dict(t=50, b=120))
        st.plotly_chart(fig2, use_container_width=True)

        ##  Gráfico 3: Porcentaje general de penales cometidos vs conseguidos
        total_favor = penales["a favor"].sum()
        total_contra = penales["en contra"].sum()
        total_penales = total_favor + total_contra

        df_pie = pd.DataFrame({
            "Tipo": ["Penales a Favor", "Penales en Contra"],
            "Cantidad": [total_favor, total_contra]
        })

        st.subheader("📈 Porcentaje Global de Penales")
        fig3 = px.pie(
            df_pie,
            names="Tipo",
            values="Cantidad",
            title="Distribución Total de Penales del Equipo",
            color="Tipo",
            color_discrete_map={
                "Penales a Favor": "#28A745",
                "Penales en Contra": "#8F1B30"
            },
            hole=0.3
        )
        ...
        st.markdown("### Qué situaciones generan más penales?")
        situacion_top = penales.sort_values("En contra", ascending=False).iloc[0]
        st.markdown(
            f"- La situación con más penales en contra fue **{situacion_top['Situación']}** con **{situacion_top['En contra']} penales**.\n"
            f"- El equipo consiguió **{total_favor} penales a favor** y cometió **{total_contra} penales en contra** en total."
        )

    else:
        st.warning("❗ La hoja 'Penales' no contiene las columnas esperadas: 'Situación', 'A favor', 'En contra'.")

except Exception as e:
    st.error(f"Error al cargar la hoja de Penales: {e}")
