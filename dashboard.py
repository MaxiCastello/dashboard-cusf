...
DASHBOARD_CSS = """
<style>
.block-container{ padding-top: 2.6rem; padding-bottom: .8rem; }
h1{ margin: 0 0 .6rem 0 !important; font-weight: 800; }
...
.card h4{
  margin:0 0 6px 0;font-weight:700;min-height:28px;
...
.kpi .lbl{ opacity:.95; font-size:14px; margin-top:6px; font-weight:600 }

.switch-head{ display:flex; align-items:center; justify-content:space-between; gap:12px; margin-bottom:6px; }
.switch-head h4{ margin:0; font-weight:700; }

.switch-head .stSelectbox > div{ min-width:220px; }
.switch-head .stSelectbox label{ display:none; } /* escondo label vacío */
    ...
    if 'fig_total' in locals() and fig_total is not None:
       fig_total.update_layout(height=(h_small + 120),
                               margin=dict(l=140 if not modo_celular else 90,
                                        r=40, t=40, b=10))
       left, mid, right = st.columns([0.7, 0.90, 0.7])
       with mid:
