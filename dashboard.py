...
def fig_to_img(fig, w=900, h=520, scale=3, width_pt=180):
...
def generar_informe_pdf(
    ...
    def colw(n, gap=6):  # ancho de cada columna para n gráficos por fila
        return (USABLE_W - gap*(n-1)) / n

    W_FULL  = USABLE_W              # una sola imagen
    W_HALF  = colw(2) * 1.1         # dos por fila
    W_THIRD = colw(3) * 1.25        # tres por fila

    # Reduce márgenes internos y baja la leyenda
    def _for_pdf(fig, title_size=12, top=30):
        fig.update_layout(
            margin=dict(l=0, r=0, t=top, b=10),
