...
def generar_informe(datos, nombre_archivo="informe_uni.pdf"):
    buffer = io.BytesIO()
    ...
    elementos.append(Paragraph("📊 Informe Temporada 2025 - Universitario de Santa Fe", estilos['Title']))
    elementos.append(Spacer(1, 12))

    # Texto inicial
    elementos.append(Paragraph("Este informe resume las estadísticas principales de la temporada 2025.", estilos['Normal']))
    elementos.append(Spacer(1, 12))

    # Tabla de resumen (ejemplo con tackles)
    data_tabla = [["Jugador", "Tackles", "Errados", "Efectividad (%)"]]
...
try:
    ...
    if "df_sumado" in locals():  # chequeamos que el df exista
        buffer = generar_informe(df_sumado)
        st.download_button(
            label="📥 Descargar Informe en PDF",
            data=buffer,
            file_name="Informe_Universitario.pdf",
