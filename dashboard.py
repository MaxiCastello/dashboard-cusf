...
if archivos:
    ...
    for archivo in archivos:
        ...
        with st.expander(f"📁 Datos del archivo: {os.path.basename(archivo)}", expanded=expandir_todo):
            ...
            etiquetas_dict = resumen.set_index("jugador")[["etiqueta", "total"]].to_dict(orient="index")

            
            df_plot["texto"] = df_plot.apply(
                lambda row: etiquetas_dict[row["jugador"]]["etiqueta"] if row["cantidad"] == etiquetas_dict[row["jugador"]]["total"] else "",
                axis=1
                )
